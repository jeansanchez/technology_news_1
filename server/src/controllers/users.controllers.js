...
export const updateUsers = async (req, res) => {
  ...
  const [result] = await pool.query(
    "UPDATE admins SET name = IFNULL(?, name), lastname = IFNULL(?, lastname), username = IFNULL(?, username), email = IFNULL(?, email), password = IFNULL(?, password), role = IFNULL(?, role) WHERE id = ?",    [name, lastname, username, email, password, role, id]
  );
  if (result.affectedRows === 0)
    return res.status(404).json({
      message: "User not found",
